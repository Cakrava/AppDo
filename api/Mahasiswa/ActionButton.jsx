...
const ActionButton = ({nim}) => {
  ...
  const actionDeleteData = async () => {
    ...
    Alert.alert(
      'Konfirmasi',
      'Apakah ang yakin ma apuih data mahasiswa ko?',
      [
        {
          text: 'Batal',
          onPress: () => console.log('ndk jadi apuih'),
          style: 'cancel',
        },
        {
          text: 'OK',
            ...
            fetch(`${apiUrl}mahasiswa/${nim}`, {
              ...
              .then(response => {
                if (response.status === 200) {
                  Alert.alert('', 'Data mahasiswa alah tahapuih!!!', [
                    {
                      text: 'Ok',
                      onPress: () =>
                        navigation.navigate('DataMahasiswa', {
                          dataAdded: true,
                        }),
                    },
                  ]);
                } else {
                  console.log('Gagal apuih data');
                }
              })
              .catch(error => {
                console.error('Ado kesalahan:', error);
              });
          },
        },
      ],
  ...
  const showFormEdit = () => {
    navigation.navigate('FormEdit', {nim: nim});
  };
