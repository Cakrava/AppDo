...
export default function ModalDataMatakuliah({
  ...
  const pilihDataMatakuliah = item => {
    onMatakuliahSelected(item.kode_2020023, item.matkul_2020023);
  };
  ...
  const renderItemMatakuliah = ({item}) => (
    <TouchableOpacity
      ...
      <Text>{item.matkul_2020023}</Text>
      <Text>{item.kode_2020023}</Text>
    </TouchableOpacity>
  );
  return (
    <Overlay
      ...
      <View style={styles.searchContainer}>
        <TextInput
          ...
          placeholder="Cari berdasarkan Kode/Nama Matakuliah"
          value={search}
