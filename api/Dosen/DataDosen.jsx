...
export default function DataDosen() {
  ...
  const fetchDataDosen = async (pageNumber = 1, searchQuery = search) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://valdo.mytugas.my.id/api/dosen/?page=${pageNumber}&search=${searchQuery}`,
        {
          headers: {
  ...
  const renderItemDosen = ({item}) => {
    const showDetailData = () => {
      navigation.navigate('DetailDosen', {nidn_2020023: item.nidn_2020023});
    };

    return (
      <TouchableOpacity style={styles.item} onPress={showDetailData}>
        <Text style={styles.titleNidn}>{item.nidn_2020023}</Text>
        <Text style={styles.text}>{item.nama_lengkap_2020023}</Text>
  ...
  return (
    <View style={styles.container}>
      <TextInput
        ...
        placeholder="Cari berdasarkan Nidn Dosen atau Nama Dosen"
        value={search}
      ...
      <FlatList
        ...
        keyExtractor={item => item.nidn_2020023}
